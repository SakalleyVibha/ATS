...
export class UserRolePermissionComponent {
 ...
 account_id:number
  ...
  getRoles(){
    ...
    this.api.allPostMethod("userRole/getUsersRoles",{account_id: this.account_id})
    .pipe(map((res:any)=>{
  ...
  getRolePermissionList(){
    this.communicate.isLoaderLoad.next(true);
    this.api.allPostMethod('rolepermission/getrolepermissionsbyAccount',{account_id: this.account_id}).subscribe({
      next: (res:any)=>{
          if(!res.error){
            this.getRoles();
            this.communicate.isLoaderLoad.next(false);
  ...
  onApply(){
    ...
    this.communicate.isLoaderLoad.next(true);
    this.api.allPostMethod('rolepermission/addEditRolePermissionByAccount',payload).subscribe({
      next: (res:any)=>{
           if(!res.error){
  ...
  changeBox(role:any,permission:any){
    let index = this.rolePermission.findIndex((v:any) => v.role_id == role.id && v.account_id == role.account_id && v.section_id == permission.section_id && v.module_id == permission.module_id && v.permission_id == permission.id);
     if(index == -1){
       this.rolePermission.push({role_id: role.id,account_id: role.account_id,module_id: permission.module_id, section_id: permission.section_id, permission_id: permission.id})
     }else{
    ...
    checkPermission(role:any,permission:any){
      return this.rolePermission.findIndex((v:any) =>  v.deleted != 1 && v.role_id == role.id && v.account_id == role.account_id && v.section_id == permission.section_id && v.module_id == permission.module_id && v.permission_id == permission.id) != -1
    }
    
    checkAll(role:any){
      role['selectAll'] = !role['selectAll'];
      this.allPemissions.forEach((p:any)=>{
        let index = this.rolePermission.findIndex((v:any) => v.role_id == role.id && v.account_id == role.account_id && v.section_id == p.section_id && v.module_id == p.module_id && v.permission_id == p.id);

        if(index !== -1 && this.rolePermission[index].id){
          this.rolePermission[index]['deleted'] =  !role['selectAll'];
        }else if(role['selectAll'] && index == -1){
            this.rolePermission.push({role_id: role.id,account_id: role.account_id,module_id: p.module_id, section_id: p.section_id, permission_id: p.id})
        }else if(!role['selectAll'] && index !== -1){
             this.rolePermission.splice(index,1)
        }
      })
    ...
    checkSelectAll(role:any){
      role['selectAll'] = this.rolePermission.filter((rp:any) => rp.role_id == role.id && rp.account_id == role.account_id && !rp.deleted).length == this.allPemissions.length;
    }
}
