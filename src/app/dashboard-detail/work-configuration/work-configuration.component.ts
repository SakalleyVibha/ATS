...
@Component({
  selector: 'app-work-configuration',
  templateUrl: './work-configuration.component.html',
  styleUrl: './work-configuration.component.css'
})
export class WorkConfigurationComponent {


  workConfigurationList = signal<Array<any>>([])
  ...
  getWorkConfigurationList() {
    this.communicate.isLoaderLoad.next(true);
    this.api.allPostMethod('work-configurations/getList', this.reqObj).subscribe((res: any) => {
      this.communicate.isLoaderLoad.next(false);
      if (res['error'] != true) {
        if ((res['data'] && res['data'].length > 0)) {
          if (this.reqObj.pageNumber == 1) {
            this.workConfigurationList.set(res['data']);
          } else
            this.workConfigurationList.update(x => {
              return [...x, ...res['data']]
            })
          this.totalPages = res['totalPages'];
        } else {
          this.workConfigurationList.set([]);
        }
      }
    });
  }
